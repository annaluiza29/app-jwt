...
const NomeC = (request, response) => {
  ...
  try {
    ...
    response.json({ message: "O nome completo da criadora é: Anna Luiza Mariano"});
  } catch (error) {
    return response.status(403).json({ message: "Token inválido ou expirado" });
  }
};

const DataNasc = (request, response) => {
  const token = request.headers["authorization"];
  ...
  try {
    ...
    response.json({ message: "A data de nascimento da criadora é: 29/09/2005"});
  } catch (error) {
    return response.status(403).json({ message: "Token inválido ou expirado" });
  }
};

module.exports = { login, protectedContent, NomeC, DataNasc};
